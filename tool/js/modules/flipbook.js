import { getPdfFlipbookTemplate } from './template.js';
...
class PdfFlipbook {
    ...
    createBlackPageElement(pageAspectRatio) {
        const pageWidth = 420;
        const pageHeight = this.getPageHeightForWidth(pageWidth, pageAspectRatio);
        const canvas = document.createElement('canvas');
        const context = canvas.getContext('2d', { alpha: false });

        canvas.width = pageWidth;
        canvas.height = pageHeight;
        canvas.setAttribute('aria-label', 'Black page');
    ...
    async render() {
        ...
        try {
            ...
            const pdf = await getDocument(this.pdfUrl);
            ...
            if (pages.length > 0) {
                pages.unshift(this.createBlackPageElement(this.maxPageAspectRatio));
                pages.push(this.createBlackPageElement(this.maxPageAspectRatio));
            }
...
export { PdfFlipbook, createPdfFlipbook };